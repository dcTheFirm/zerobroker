...
function buildMongoFilter(filters: SearchFilters = {}): Filter<PropertyDocument> {
  const normalized = normalizeFilters(filters)
  const mongoFilter: Filter<PropertyDocument> = { type: 'buy' }
...
function formatBuyPrice(price: number) {
  if (price >= 10000000) {
...
function toProperty(listing: ListingInput): Property {
  return {
    ...
    area: listing.area,
    price: listing.price,
    priceLabel: formatBuyPrice(listing.price),
    ...
    lat: 0,
    lng: 0,
    description: 'Owner-submitted sale listing.',
    listedAt: new Date().toISOString().slice(0, 10),
    ownerVerified: false,
  }
}

export async function getBuyProperties(filters: SearchFilters = {}): Promise<Property[]> {
...
export async function listBuyProperties(filters: SearchFilters = {}, page = 1, limit = DEFAULT_LIMIT) {
  const collection = await getPropertiesCollection()
...
export async function getBuyPropertyById(id: string): Promise<Property | null> {
  const collection = await getPropertiesCollection()

  if (collection) {
    const property = await collection.findOne({ id, type: 'buy' }, { projection: { _id: 0 } })
    return property as Property | null
  }

  return properties.find((property) => property.id === id && property.type === 'buy') ?? null
}

export async function createBuyProperty(listing: ListingInput): Promise<Property> {
