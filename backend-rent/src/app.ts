...
import { z } from 'zod'
import { getRentProperties, getRentPropertyById, listRentProperties } from './services/propertyService.js'
...
app.get('/api/rent/properties', (req, res) => {
  ...
  res.json(listRentProperties(filters, page, limit))
})

app.post('/api/rent/properties', (req, res) => {
  const parsed = listingSchema.safeParse(req.body)
  if (!parsed.success) {
    return res.status(400).json({ error: 'Invalid listing details.' })
  }

  return res.status(201).json({ message: 'Rent property listing submitted successfully.' })
})

app.post('/api/rent/properties/:id/contact', (req, res) => {
  return res.status(200).json({ message: 'Contact request received. The owner will reach out shortly.' })
})

app.post('/api/rent/properties/:id/visit', (req, res) => {
  return res.status(200).json({ message: 'Visit request received. The owner will contact you to confirm the schedule.' })
})

app.get('/api/rent/properties/:id', (req, res) => {
  const property = getRentPropertyById(req.params.id)
  if (!property) {
